...
from backend.utils.pile import Pile
...
def init_game(num_players):
    ...
    if players:
        for i, player in enumerate(players):
            player.set_turn(i == 0)
            if player.is_turn:
                return jsonify({"message": f"Game initialized with {num_players} players", "current_turn": i}), 200
...
def hit_bell(player_id):
    ...
    if 0 <= player_id < len(players):
        all_player_top_cards = [player.get_top_card() for player in players if player.get_top_card()]
        ...
        if pile.check_bell_hit(all_player_top_cards):
            ...
            return jsonify({"message": "Bell hit incorrectly. No cards collected."}), 200
    else:
